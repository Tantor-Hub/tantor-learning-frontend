...
export function SessionSelector({ selectedSessionId, onSessionChange }: SessionSelectorProps) {
  ...
  // When sessions data is loaded, update localStorage and validate selected session
  useEffect(() => {
    if (sessions?.data) {
      localStorage.setItem("userSessions", JSON.stringify(sessions.data));
      setLocalSessions(sessions.data);

      // Check if the currently selected session exists in the fetched data
      const selectedExists = sessions.data.some(
        (s: any) => s.trainingSession.id === selectedSessionId
      );

      if (!selectedSessionId || !selectedExists) {
        // If no session selected or selected session doesn't exist, set first available
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="mt-3 text-[15px]">
              {hasNoSessions
                ? "Il n'y a pas de sessions dans lesquelles vous êtes enregistré. Allez voir la liste des formations pour vous inscrire."
