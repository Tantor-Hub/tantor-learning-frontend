...
export default function InstructorsTab({ title, description }: InstructorsTabProps) {
  const {
    data: apiData,
    isLoading,
    isError,
    refetch,
  } = useListUserByRoleQuery({ role: UserRole.INSTRUCTOR });
      ...
      <div className="flex justify-center items-center h-[calc(100vh-160px)]">
        <div className="flex flex-col items-center gap-4 p-4 max-w-md text-center">
          <p className="text-destructive">
            {"Impossible de charger les formateurs. Veuillez réessayer plus tard."}
          </p>
  ...
  const instructors = apiData?.data ?? [];
