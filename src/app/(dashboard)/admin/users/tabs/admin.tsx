...
export default function AdminTab({ title, description }: AdminTabProps) {
  const {
    data: apiData,
    isLoading,
    isError,
    refetch,
  } = useListUserByRoleQuery({ role: UserRole.ADMIN });
      ...
      <div className="flex justify-center items-center h-[calc(100vh-160px)]">
        <div className="flex flex-col items-center gap-4 p-4 max-w-md text-center">
          <p className="text-destructive">
            {"Impossible de charger les administrateurs. Veuillez réessayer plus tard."}
          </p>
    ...
    <div className="bg-white border flex flex-col rounded-md gap-10 p-5 md:p-10">
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[1000px]">
          {admins.length > 0 ? (
            <Table>
              <TableCaption>Liste de tous les administrateurs</TableCaption>
              <TableHeader className="border">
                <TableRow>
                  <TableHead>Nom</TableHead>
                  <TableHead>Email</TableHead>
            ...
            <div className="text-center py-10 text-gray-500">Aucun administrateur trouvé</div>
          )}
        </div>
      </div>
    </div>
