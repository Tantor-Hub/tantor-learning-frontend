...
import { Check, X, Loader2, ChevronsUpDown, Edit } from "lucide-react";
...
export function UpdateEvaluationModal({
  evaluationId,
  courseId,
  isOpen,
  onClose,
  ...
  } = useGetStudentEvaluationByIdQuery({ id: evaluationId }, { skip: !isOpen || !evaluationId });
  ...
  const [updateEvaluation, { isLoading: isUpdating }] = useUpdateStudentEvaluationMutation();
  ...
  // Populate form when evaluation data is loaded
  useEffect(() => {
    if (evaluationData?.data) {
      const evalData = evaluationData.data;
      setTitle(evalData.title || "");
      setDescription(evalData.description || "");
      setType(evalData.type || StudentevaluationType.QUIZ);
      setPoints(evalData.points || 100);
      setSubmittiondate(
        evalData.submittiondate ? new Date(evalData.submittiondate).toISOString().slice(0, 16) : ""
      );
      setBeginningTime(evalData.beginningTime?.slice(0, 5) || "");
  ...
  // Handle evaluation loading error
  useEffect(() => {
    if (evaluationError) {
  ...
  const handleUpdate = async () => {
    ...
    const toastId = toast.loading("Mise à jour de l'évaluation en cours...");
    ...
    try {
      ...
      await updateEvaluation({ id: evaluationId, body: evaluationData }).unwrap();
    ...
    return (
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Mise à jour de l'évaluation</DialogTitle>
          </DialogHeader>
