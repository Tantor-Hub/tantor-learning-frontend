...
import { useListSessionQuery } from "@/lib/apis/secretary/session-secretary-api";
import {
  Select,
  SelectContent,
  SelectItem,
      ...
      <TabsContent value="courses">
        <div className="overflow-x-auto p-8 my-5 border rounded-md">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-primary text-xl font-semibold mb-3">Liste de tous les cours</h2>
            </div>
          </div>
          <div>
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="w-[300px]">
              ...
              >
                ...
                <SelectContent>
                  {listSessions.map((session: any) => (
                    <SelectItem key={session.id} value={session.id.toString()}>
                      {session.title} - {session.designation}
                    </SelectItem>
                  ))}
                </SelectContent> 
              </Select>
